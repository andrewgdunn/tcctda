...
public class ErrorFinder {
	...
	public static Map<String, Value> timeOfError(Sensor sensor) {
		...
		map = timeOfStuckError(sensor);
		if(map.size()>0)
			return map;
		
		// next look for intermittent
		map = timeOfIntermittentError(sensor);
		if(map.size()>0)
			return map;
				
		// then abrupt
		map = timeOfAbruptError(sensor);
		if(map.size()>0)
			return map;
	...
	private static Map<String, Value> timeOfStuckError(Sensor sensor) {
		Map<String, Value> map = new HashMap();
	...
	private static Map<String, Value> timeOfAbruptError(Sensor sensor) {
		Map<String, Value> map = new HashMap();
...
//---------------------------------------------------------------------------------------------
	...
	private static Map<String, Value> timeOfIntermittentError(Sensor sensor) {
		Map<String, Value> map = new HashMap();
		
		Map<String, Value> abrupt = timeOfAbruptError(sensor);
...
//---------------------------------------------------------------------------------------------
	...
	private static Map<String, Value> timeOfDriftError(Sensor sensor) {
		Map<String, Value> map = new HashMap();
		
		// smooth data (to hopefully remove noise)
		int smoothRange = 50;
		int smoothCount = smoothRange / 2;
		double sum=0;
		double[] smoothedData = new double[sensor.data.size()];
		// calculate initial sum
		for(int i=0; i<smoothRange/2; i++) {
			double val = ((RealValue)sensor.data.elementAt(i)).get();
			sum += val;
		}
