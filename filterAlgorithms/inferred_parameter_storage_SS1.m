function [inferred_parameters,data_restart] = inferred_parameter_storage_SS1(data_start,N_samples,inferred_parameters,t_inj_loc_guess,sensor_hz,data_ignore,t_hist,X_KF)
parameters = 6;
...
X = X_KF(data_start:t_inj_loc_used);
n = length(T);
...
inferred_parameters(5,index) = mean(X_KF(data_start:t_inj_loc_used));
inferred_parameters(6,index) = (n*T*X' - sum(T)*sum(X))/(n*sum(T.^2) - sum(T)^2);
